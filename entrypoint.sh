...
if [ -z "${SUBSPACE_NAMESERVERS-}" ]; then
  export SUBSPACE_NAMESERVERS="1.1.1.1"
fi
...
if [ "$SUBSPACE_IPV4_NAT_ENABLED" == "0" ]; then
  echo "One of envionment variables SUBSPACE_IPV4_NAT_ENABLED must be set to 1."
  echo "Got SUBSPACE_IPV4_NAT_ENABLED=$SUBSPACE_IPV4_NAT_ENABLED"
  exit 1
fi
